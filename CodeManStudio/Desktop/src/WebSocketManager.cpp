...
namespace CMS {
...

	{
		...
		if (!server_->listen(QHostAddress::Any, 4030)) {
			ServerLogger_->error("Failed to start WebSocket server on port 4030: {}", server_->errorString().toStdString());
		}
		else {
	...
	void WebSocketManager::OnDisconnectedFromServer() {
		ClientLogger_->info("=== Disconnect to server ===");

		client_->abort();

	}
	void WebSocketManager::MessageFromServer(const QString& message) {
	...
	void WebSocketManager::MessageFromClient(const QString& message) {
